...
1. **Copy the templates**: `cp -r knowledge.example knowledge`
2. **Fill in `knowledge/profile.md`, `education.md`, and `skills.md`** with
   ...
   fork public.
3. **Delete or rewrite `knowledge/rules.md`** — it holds per-person
   selection preferences (which roles to always include, where to archive
   finished PDFs). The skill works fine without it.
4. **Install tectonic** — `brew install tectonic` (macOS) or see
   https://tectonic-typesetting.github.io.
5. Open the folder in Claude Code and run `/tailor <jd-url-or-file>`.
...
[Tectonic](https://tectonic-typesetting.github.io) compiles the LaTeX — it
downloads required packages on demand, so no full TeX distribution is needed:

```bash
brew install tectonic
```

`pdftotext` (from poppler) is used for the ATS extraction check:
